  ...
  return (
    <AppShell>
      {!page || !canView(page, user) ? (
        ...
        <PageBody page={page} tab={tab} setTab={setTab} />
      )}
    </AppShell>
  );
}

function buildCrumb(page: Page): Page[] {
  const chain: Page[] = [];
...
function PageBody({
  ...
  const crumb = buildCrumb(page);
  const depth = crumb.length - 1;
  const atMaxDepth = depth >= 2;
  const children = getChildren(page.id, user);
  const backlinks = getBacklinks(page.id, user);
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-start gap-5 mb-5">
        ...
        <button className="border border-border rounded-s px-4 py-2 text-sm hover:bg-surface-2 shrink-0">編集する</button>
      </div>
      ...
      {tab === "body" ? (
        <>
          ...
          <div
            ...
            dangerouslySetInnerHTML={{ __html: renderMarkdown(page.body, page.id) }}
          />
          ...
          <div className="mt-4.5">
            {atMaxDepth ? (
              ...
              <button className="border border-border rounded-s px-3.5 py-1.5 text-[0.82rem] hover:bg-surface-2">+ 子ページを追加</button>
            )}
          </div>
